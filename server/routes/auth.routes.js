...
router.post("/signup", async (req, res, next) => {
    ...
    if(passwordRegex.test(password) === false){
        return res.status(400).json({errorMessage: "La contraseña no es suficientemente fuerte. Tiene que tener más de 8 caracteres, al menos una letra mayúscula, una minúscula y otro caracter"})
    }
    ...
    try {
       ...
       if(foundUser) {
        return res.status(400).json({errorMessage: "El email ya existe en nuestra base de datos"})
       }
...
router.post("/login", async (req, res, next) => {
    ...
    try {
        ...
        if(!foundUser){
            return res.status(400).json({errorMessage: "El usuario no existe en la base de datos"})
        }
